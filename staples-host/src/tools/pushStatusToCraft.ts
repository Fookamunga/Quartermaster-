...
import { daysSince } from "../replenishment.js";
...
function groupByUrgency(items: Item[]): Section[] {
  const overdue: Item[] = [];
  const due: Item[] = [];
  const stocked: Item[] = [];
...
function buildStatusLines(items: Item[]): string[] {
  const lines: string[] = [
  ...
  for (const section of groupByUrgency(items)) {
...
export function registerPushStatusToCraft(server: McpServer): void {
  server.registerTool(
    ...
    async () => {
      ...
      const lines = buildStatusLines(db.items);
      ...
      const counts = Object.fromEntries(
        groupByUrgency(db.items).map((s) => [s.title, s.items.length]),
      );
